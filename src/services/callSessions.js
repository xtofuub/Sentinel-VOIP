...
export async function listCallSessions(userId) {
  ...
  const { data, error } = await supabase
    .from("call_sessions")
    .select("id,user_id,scenario_id,scenario_title,locale_code,recipient_name,recipient_phone,scheduled_for,status,attempt_count,upstream_did,upstream_uid,upstream_task_id,failure_reason,launched_at,created_at,updated_at")
    .eq("user_id", userId)
...
export function callSessionToActivity(session) {
  ...
  return {
    ...
    cou: String(session.locale_code || "fi").toLowerCase().slice(0, 2),
